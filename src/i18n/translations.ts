export const translations = {
  en: {
    nav: {
      home: "Home",
      services: "Services",
      about: "About",
      contact: "Contact",
      bookConsultation: "Book a call",
    },
    hero: {
      eyebrow: "Boutique ESG advisory",
      title: "ESG that fits the way your business actually runs.",
      subtitle: "Sustavision helps SMEs turn ESG requirements, customer expectations, and reporting deadlines into clear, manageable action - without the overhead of a large consultancy.",
      positioning: "Clear ESG direction. Practical execution.",
      cta: "Book an introductory call",
      secondaryCta: "Explore services",
      altHeadline1: "Turn ESG requirements into clear, manageable action.",
      altHeadline2: "Boutique ESG advisory for SMEs.",
      altHeadline3: "Senior ESG support, sized for SMEs.",
    },
    trustStrip: {
      items: [
        "Boutique ESG advisory",
        "Founder-led, senior delivery",
        "Reporting · Carbon · Supply chain · Implementation",
        "Tailor-built to your company's needs",
      ],
    },
    triggers: {
      eyebrow: "Where we support",
      title: "Where we support",
      description: "Most clients reach out when ESG requirements, customer expectations or reporting deadlines start to feel urgent. We help them quickly.",
      exploreLabel: "Explore",
      closeLabel: "Close",
      items: [
        {
          title: "Customer, platform or tender requests",
          description: "A customer, platform or tender asks for ESG information. Get clear, credible support for EcoVadis, CDP, supplier questionnaires, tender responses and sustainability-data requests - without creating unnecessary internal workload.",
          imageAlt: "Two professionals reviewing supplier documents and a digital questionnaire",
        },
        {
          title: "Regulatory and certification pressure",
          description: "CSRD, VSME, sustainability labels and changing requirements can be difficult to navigate. Understand what applies to your business, what customers may expect, and where to focus first.",
          imageAlt: "A sustainability advisor reviewing structured standards documents",
        },
        {
          title: "Turn sustainability ambitions into practical action",
          description: "Measure your footprint, set realistic priorities and turn sustainability ambitions into progress. Support can include carbon footprints, Scope 1-3, supplier engagement, reduction plans and SBTi-aligned target support.",
          imageAlt: "Operations professionals reviewing sustainability data in a modern production facility",
        },
      ],
    },
    serviceRoutes: {
      title: "How we help you move forward",
      description: "Whether you need to respond to a customer request, make sense of ESG expectations or build practical momentum, Sustavision helps you focus on what matters and take the next step with confidence.",
      cta: "Explore our services",
      cards: [
        {
          heading: "Get clear on what matters",
          text: "Understand the ESG issues, requirements and priorities most relevant to your business.",
          supporting: "Understand your ESG priorities and get a focused roadmap you can actually implement.",
        },
        {
          heading: "Respond with confidence",
          text: "Prepare credible responses to customer, platform and tender requirements.",
          supporting: "Clear, well-structured answers for EcoVadis, CDP, customer and tender ESG requests.",
        },
        {
          heading: "Turn plans into progress",
          text: "Put practical carbon, supplier and ESG actions into place.",
          supporting: "Measure your footprint, set priorities and take practical steps on carbon and suppliers.",
        },
      ],
    },
    whySustavision: {
      title: "A different kind of ESG support",
      description: "Sustavision is built for lean teams that need credible ESG support without consultant overhead or unnecessary complexity.",
      points: [
        {
          heading: "Senior, founder-led support",
          text: "Direct access to an experienced ESG advisor who can both guide and execute. No handovers to junior consultants, no layered project structures. You work with someone who understands both the technical details and the business reality - and can take full responsibility for getting ESG work done.",
        },
        {
          heading: "Built for SMEs",
          text: "Proportionate scope, practical tools and limited internal burden. Support is designed around your available time, resources and priorities - not a generic, large-consultancy playbook.",
        },
        {
          heading: "Operational ESG experience",
          text: "Credible support where customer requests, regulation, carbon and supply chains meet. Experience across distribution, manufacturing and logistics-linked contexts, with a focus on what is useful and implementable.",
        },
      ],
    },
    services: {
      eyebrow: "Service areas",
      title: "Five practical ways we work with SMEs.",
      description: "Broad enough to cover the full ESG agenda, focused enough to deliver outcomes. Each area can be engaged on its own or as part of a wider programme.",
      items: [
        {
          title: "ESG strategy & roadmap",
          covers: "Materiality, ambition setting, prioritisation, and a sequenced plan tied to your business reality.",
          when: "You need a clear direction, internal alignment, or a credible answer to investors, customers, or your board.",
          result: "A focused ESG strategy and roadmap your team can actually execute.",
        },
        {
          title: "Reporting readiness & disclosure support",
          covers: "CSRD and VSME scoping, double materiality, data architecture, narrative structure, and assurance preparation.",
          when: "A reporting obligation is approaching, or your current disclosure is fragmented, manual, or hard to defend.",
          result: "A reporting blueprint, clean data flows, and a disclosure that holds up under scrutiny.",
        },
        {
          title: "Carbon footprinting & climate action",
          covers: "Scope 1, 2, 3 inventories, hotspot analysis, science-aligned targets, reduction roadmaps, and climate risk.",
          when: "Customers, regulators, or financiers are asking for emissions data - or you want a credible decarbonisation plan.",
          result: "A defensible footprint and a prioritised set of reduction moves with owners and timelines.",
        },
        {
          title: "Customer, supplier & framework support",
          covers: "EcoVadis, CDP, customer ESG questionnaires, supplier engagement, due diligence, and Scope 3 work.",
          when: "Commercial requests are stacking up, or supplier ESG is becoming a recurring blocker.",
          result: "Stronger scores, faster answers, and a value-chain approach proportionate to your leverage.",
        },
        {
          title: "ESG implementation & KPI follow-through",
          covers: "Action plans, KPI design, governance, internal routines, and hands-on programme management.",
          when: "A strategy or report exists on paper, but execution is stalling or buried under day-to-day work.",
          result: "ESG that actually moves - owned by your team, supported by ours, measured every quarter.",
        },
      ],
      detailCovers: "What it covers",
      detailWhen: "When you need it",
      detailResult: "What you get",
    },
    process: {
      eyebrow: "How we work",
      title: "A calm, structured way to make ESG manageable.",
      description: "Five steps, used end-to-end or as standalone engagements. Iterative by design, because ESG is not a one-off project.",
      cycleCta: "Iterative by design",
      cycleTitle: "Built to repeat, not to restart.",
      cycleDesc1: "ESG expectations, regulation, and stakeholder questions evolve every year. We design each engagement so the work compounds - your baseline, materiality, KPIs, and disclosures get sharper with every cycle.",
      cycleDesc2: "That means lower cost per cycle, faster answers to customers and auditors, and a quiet, durable ESG capability inside your business.",
      objective: "Objective",
      deliverables: "Deliverables",
      standalone: "Available as standalone",
      steps: [
        {
          title: "Understand your context",
          shortDesc: "Business model, stakeholders, regulatory exposure, and what is genuinely material.",
          objective: "Build a sharp picture of your business and the ESG pressures that actually matter to it.",
          deliverables: ["Context & stakeholder map", "Regulatory exposure summary", "Working hypothesis on priorities"],
        },
        {
          title: "Assess the current baseline",
          shortDesc: "Maturity check, data review, supplier and peer benchmarking.",
          objective: "Establish where you stand today - honestly, without inflating or under-selling the work already done.",
          deliverables: ["Maturity assessment", "Baseline data review", "Peer & supplier benchmarking"],
        },
        {
          title: "Prioritise what matters most",
          shortDesc: "Materiality, ambition setting, and a sequenced roadmap.",
          objective: "Decide what to act on, in what order, and at what depth - based on impact and feasibility.",
          deliverables: ["Materiality view", "ESG strategy & ambitions", "Sequenced roadmap"],
        },
        {
          title: "Implement practical next steps",
          shortDesc: "Action plans, KPIs, governance, and hands-on programme support.",
          objective: "Translate strategy into routines your team can run without us being in the room every week.",
          deliverables: ["Action plans with owners", "KPI design & governance", "Internal capability building"],
        },
        {
          title: "Support reporting & follow-through",
          shortDesc: "Reporting structure, disclosure, customer & framework responses.",
          objective: "Turn the work into clear, defensible communication - internally, to customers, and in formal disclosures.",
          deliverables: ["Reporting blueprint", "CSRD / VSME alignment", "Customer & framework responses"],
        },
      ],
    },
    why: {
      eyebrow: "Why Sustavision",
      title: "Boutique quality, sized for the way SMEs actually operate.",
      reasons: [
        { title: "Tailor-built to your company's needs", description: "Every engagement is shaped around your sector, size, and maturity - no off-the-shelf templates or recycled playbooks." },
        { title: "Practical over theoretical", description: "Frameworks where they help, plain action where they don't. We optimise for what your team can actually run." },
        { title: "Built for lean teams", description: "We adapt to companies without a dedicated ESG department - minimal internal overhead, maximum forward motion." },
        { title: "Multiple entry points", description: "Strategy, reporting, carbon, supply chain, or implementation. Start where the pressure is, expand when it pays off." },
        { title: "EU & market fluency", description: "CSRD, VSME, EcoVadis, CDP, customer questionnaires - current expectations, current language, current evidence." },
        { title: "Calm, no-nonsense execution", description: "Direct communication, honest scoping, and a working style that respects your time and your business." },
      ],
    },
    experience: {
      title: "Experience across operational sectors",
      description: "We have delivered ESG support in distribution, manufacturing and logistics-linked contexts - among others. The sectors below are examples, not limits.",
      sectors: [
        { title: "Chemicals & distribution" },
        { title: "Food & drinks distribution" },
        { title: "Port & industrial operations" },
        { title: "Manufacturing" },
      ],
    },
    finalCta: {
      title: "Not sure where to begin?",
      description: "In a 30-minute introductory call, we identify the real priority and whether Sustavision is the right fit. No obligation, no generic sales pitch - just a clear view of what makes sense for your business.",
      cta: "Book a 30-minute introductory call",
      secondary: "Or email Thomas at thomas@sustavision.com",
    },
    footer: {
      tagline: "Boutique ESG advisory for SMEs.",
      positioning: "Clear ESG direction. Practical execution.",
      quickLinks: "Navigate",
      contact: "Contact",
      email: "thomas@sustavision.com",
      phone: "+32 495 632 266",
      rights: "All rights reserved.",
    },
    about: {
      eyebrow: "About",
      title: "About Sustavision",
      bio1: "Sustavision is led by Thomas Huyghe. I help SMEs and growing companies deal with ESG questions, customer requests and reporting pressure without turning it into a big internal project.",
      bio2: "I hold an MSc in Supply Chain Management (University of Antwerp) and an MSc in Global Business & Sustainability (Rotterdam School of Management). Earlier, at BDO, I co-founded the sustainability practice and helped scale it from 2 to 13 consultants in 2.5 years. At Manuchar, I led carbon footprinting, responsible sourcing and the decarbonisation roadmap, work that contributed to the company achieving EcoVadis Platinum.",
      bio3: "Sustavision exists to make that level of ESG advisory available to SMEs and growing companies, without the layers, overhead or theoretical detour of a large consultancy.",
      bio4: "You get senior, hands-on support that is sized for lean teams and focused on what actually moves the needle for your business.",
      cta: "Book a 30-minute introductory call",
    },
    servicesPage: {
      title: "Services",
      description: "ESG is becoming part of doing business: customer questionnaires, tenders, new rules. We help you make sense of it, respond credibly and put practical actions in place, so you can focus on running your company.",
      includedLabel: "What's included",
      outcomesLabel: "Typical outcomes",
      forLabel: "Who this is for",
      discussCta: "Discuss this service",
      services: [
        {
          title: "Get clear on what matters",
          introduction: "You need to understand what ESG really means for your business. We help you map requirements, customer expectations and internal priorities, then turn this into a focused roadmap you can actually implement.",
          included: [
            "ESG maturity scan",
            "Review of customer, platform and regulatory expectations",
            "CSRD relevance check and VSME readiness",
            "Materiality assessment (full or light-touch, as needed)",
            "Clear, prioritised 12-18 month ESG roadmap",
          ],
          outcomes: [
            "Clarity on what matters now vs later",
            "A practical ESG plan aligned with your resources",
            "Confidence when talking to customers, banks or investors",
          ],
          forWhom: "Especially useful for businesses facing growing ESG questions but without a dedicated sustainability function.",
        },
        {
          title: "Respond with confidence",
          introduction: "Customers, platforms and tenders increasingly ask for ESG information. We help you prepare clear, credible responses without creating unnecessary internal workload.",
          included: [
            "Support with EcoVadis, CDP and other ESG questionnaires",
            "Customer and supplier ESG data requests",
            "Tender and RFP sustainability sections",
            "Evidence gathering and documentation support",
          ],
          outcomes: [
            "Stronger, more consistent ESG answers",
            "Organised proof and documentation",
            "Less last-minute stress around deadlines",
          ],
          forWhom: "Businesses that receive ESG questionnaires or data requests from customers, platforms or tenders and need practical, expert support.",
        },
        {
          title: "Turn plans into progress",
          introduction: "You want to move from ESG intentions to tangible actions. We help you measure your footprint, set priorities and implement practical improvements that fit your business.",
          included: [
            "Carbon footprint (Scope 1-3) and hotspot analysis",
            "Practical reduction priorities and actions",
            "Supplier engagement and data collection",
            "ESG KPIs and simple tracking routines",
            "SBTi-aligned target support (where relevant)",
          ],
          outcomes: [
            "First credible carbon baseline",
            "Clear, implementable reduction steps",
            "Better supplier ESG data and engagement",
            "Simple routines your team can maintain",
          ],
          forWhom: "Businesses that want to start measuring and improving their environmental and social impact, without overcomplicating things.",
        },
      ],
      process: {
        title: "How we work",
        description: "Structured, but focused on your reality.",
        steps: [
          { title: "Understand your situation", description: "We start with your context: customers, sector, resources and the ESG questions you are facing." },
          { title: "Set the right priorities", description: "Together we define what is relevant now, what can wait and what a realistic next step looks like." },
          { title: "Take practical next steps", description: "We help you implement the work, from questionnaires and roadmaps to carbon data and supplier engagement." },
        ],
      },
      finalCta: {
        title: "Not sure which service fits?",
        description: "In a 30-minute introductory call, we look at your situation and the ESG questions you are facing. You get a clear view of what makes sense to do next, with no obligation and no generic sales pitch.",
        cta: "Book a 30-minute introductory call",
        secondary: "Or email Thomas at thomas@sustavision.com",
      },
    },
    contact: {
      title: "Let's talk.",
      description: "Tell us where you are and what is on your plate. We will respond personally, usually within one working day.",
      formTitle: "Send a message",
      nameLabel: "Name",
      namePlaceholder: "Your name",
      emailLabel: "Email",
      emailPlaceholder: "your@company.com",
      messageLabel: "Message",
      messagePlaceholder: "Tell us briefly about your context and what you are trying to solve.",
      submit: "Send message",
      sending: "Sending…",
      success: "Message sent.",
      successDesc: "Thank you. We will get back to you personally, usually within one working day.",
      error: "Something went wrong",
      errorDesc: "Please try again, or email thomas@sustavision.com directly.",
      directContact: "Or reach out directly",
    },
  },
  nl: {
    nav: {
      home: "Home",
      services: "Diensten",
      about: "Over ons",
      contact: "Contact",
      bookConsultation: "Plan een gesprek",
    },
    hero: {
      eyebrow: "Boutique ESG-advies",
      title: "ESG dat past bij hoe uw bedrijf écht werkt.",
      subtitle: "Sustavision helpt KMO's om ESG-vereisten, klantverwachtingen en rapportagedeadlines om te zetten in heldere, beheersbare actie - zonder de overhead van een groot kantoor.",
      positioning: "Heldere ESG-richting. Praktische uitvoering.",
      cta: "Plan een kennismaking",
      secondaryCta: "Bekijk diensten",
      altHeadline1: "Zet ESG-vereisten om in heldere, beheersbare actie.",
      altHeadline2: "Boutique ESG-advies voor KMO's.",
      altHeadline3: "Ervaren ESG-ondersteuning, op KMO-maat.",
    },
    trustStrip: {
      items: [
        "Boutique ESG-advies",
        "Oprichter-geleid, senior uitvoering",
        "Rapportering · Carbon · Keten · Implementatie",
        "Op maat gemaakt voor uw bedrijf",
      ],
    },
    triggers: {
      eyebrow: "Waar wij ondersteunen",
      title: "Waar wij ondersteunen",
      description: "De meeste klanten nemen contact op wanneer ESG-vereisten, klantverwachtingen of rapporteringsdeadlines dringend beginnen te worden. Wij helpen hen snel verder.",
      exploreLabel: "Ontdek",
      closeLabel: "Sluiten",
      items: [
        {
          title: "Vragen van klanten, platformen of aanbestedingen",
          description: "Een klant, platform of aanbesteding vraagt om ESG-informatie. Krijg duidelijke, geloofwaardige ondersteuning voor EcoVadis, CDP, leveranciersvragenlijsten, aanbestedingsantwoorden en vragen om duurzaamheidsdata - zonder onnodige interne werklast te creëren.",
          imageAlt: "Twee professionals die leveranciersdocumenten en een digitale vragenlijst bekijken",
        },
        {
          title: "Druk door regelgeving en certificering",
          description: "CSRD, VSME, duurzaamheidslabels en veranderende vereisten kunnen moeilijk te doorgronden zijn. Begrijp wat voor uw bedrijf van toepassing is, wat klanten kunnen verwachten en waarop u zich eerst moet richten.",
          imageAlt: "Een duurzaamheidsadviseur die gestructureerde normdocumenten doorneemt",
        },
        {
          title: "Zet duurzaamheidsambities om in praktische actie",
          description: "Meet uw voetafdruk, bepaal realistische prioriteiten en zet duurzaamheidsambities om in vooruitgang. Ondersteuning kan bestaan uit CO2-voetafdrukken, Scope 1-3, leveranciersbetrokkenheid, reductieplannen en ondersteuning voor SBTi-afgestemde doelstellingen.",
          imageAlt: "Operationele professionals die duurzaamheidsdata bekijken in een moderne productieomgeving",
        },
      ],
    },
    serviceRoutes: {
      title: "Hoe wij u vooruit helpen",
      description: "Of u nu moet antwoorden op een klantvraag, ESG-verwachtingen wilt doorgronden of praktische vaart wilt opbouwen, Sustavision helpt u zich te richten op wat ertoe doet en met vertrouwen de volgende stap te zetten.",
      cta: "Ontdek onze diensten",
      cards: [
        {
          heading: "Helderheid over wat ertoe doet",
          text: "Begrijp de ESG-thema's, vereisten en prioriteiten die het meest relevant zijn voor uw bedrijf.",
          supporting: "Inzicht in uw ESG-prioriteiten en een gefocuste roadmap die u ook echt kunt uitvoeren.",
        },
        {
          heading: "Antwoorden met vertrouwen",
          text: "Bereid geloofwaardige antwoorden voor op vragen van klanten, platformen en aanbestedingen.",
          supporting: "Duidelijke, goed gestructureerde antwoorden voor EcoVadis, CDP, klant- en aanbestedingsvragen rond ESG.",
        },
        {
          heading: "Van plan naar vooruitgang",
          text: "Zet praktische maatregelen rond carbon, leveranciers en ESG in de praktijk.",
          supporting: "Meet uw voetafdruk, stel prioriteiten en zet concrete stappen rond carbon en leveranciers.",
        },
      ],
    },
    whySustavision: {
      title: "ESG-ondersteuning van een andere orde",
      description: "Sustavision is gebouwd voor lean teams die geloofwaardige ESG-ondersteuning zoeken zonder consultantoverhead of onnodige complexiteit.",
      points: [
        {
          heading: "Senior, founder-led ondersteuning",
          text: "Directe toegang tot een ervaren ESG-adviseur die zowel kan begeleiden als uitvoeren. Geen doorgifte aan junior consultants, geen meerlagige projectstructuren. U werkt met iemand die zowel de technische details als de bedrijfsrealiteit begrijpt en volledige verantwoordelijkheid kan opnemen om het ESG-werk af te ronden.",
        },
        {
          heading: "Gemaakt voor KMO's",
          text: "Evenredige scope, praktische tools en beperkte interne belasting. De ondersteuning is opgebouwd rond uw beschikbare tijd, middelen en prioriteiten - niet rond een generiek playbook van een groot advieskantoor.",
        },
        {
          heading: "Operationele ESG-ervaring",
          text: "Geloofwaardige ondersteuning daar waar klantvragen, regelgeving, carbon en toeleveringsketens samenkomen. Ervaring in distributie, productie en logistiek-gerelateerde contexten, met focus op wat bruikbaar en uitvoerbaar is.",
        },
      ],
    },
    services: {
      eyebrow: "Dienstendomeinen",
      title: "Vijf praktische manieren waarop we met KMO's werken.",
      description: "Breed genoeg om de volledige ESG-agenda te dekken, gefocust genoeg om resultaat te leveren. Elk domein kan apart of als onderdeel van een breder traject worden ingezet.",
      items: [
        {
          title: "ESG-strategie & roadmap",
          covers: "Materialiteit, ambitiebepaling, prioritering en een gefaseerd plan dat aansluit bij uw realiteit.",
          when: "U heeft nood aan duidelijke richting, interne afstemming, of een geloofwaardig antwoord aan investeerders, klanten of de raad van bestuur.",
          result: "Een gefocuste ESG-strategie en roadmap die uw team ook echt kan uitvoeren.",
        },
        {
          title: "Rapportagebereidheid & disclosure",
          covers: "CSRD- en VSME-scoping, dubbele materialiteit, data-architectuur, narratief en voorbereiding op assurance.",
          when: "Een rapporteringsverplichting komt eraan, of uw huidige disclosure is versnipperd, manueel of moeilijk te verdedigen.",
          result: "Een rapporteringsblauwdruk, schone datastromen en een disclosure die controle doorstaat.",
        },
        {
          title: "Carbon footprinting & klimaatactie",
          covers: "Scope 1-, 2- en 3-inventarissen, hotspot-analyse, science-based targets, reductieroadmaps en klimaatrisico.",
          when: "Klanten, regelgevers of financiers vragen emissiedata - of u wil een geloofwaardig decarbonisatieplan.",
          result: "Een verdedigbare footprint en een geprioriteerde set reductiemaatregelen met eigenaars en timing.",
        },
        {
          title: "Klant-, leveranciers- & frameworkondersteuning",
          covers: "EcoVadis, CDP, ESG-vragenlijsten van klanten, leveranciersbetrokkenheid, due diligence en Scope 3-werk.",
          when: "Commerciële vragen stapelen op, of leveranciers-ESG wordt een terugkerend struikelblok.",
          result: "Sterkere scores, snellere antwoorden en een ketenaanpak in verhouding tot uw leverage.",
        },
        {
          title: "ESG-implementatie & KPI-opvolging",
          covers: "Actieplannen, KPI-design, governance, interne routines en hands-on programmamanagement.",
          when: "Er is een strategie of rapport op papier, maar de uitvoering loopt vast tussen de dagelijkse werking.",
          result: "ESG die effectief vooruit gaat - gedragen door uw team, ondersteund door ons, elk kwartaal gemeten.",
        },
      ],
      detailCovers: "Wat het omvat",
      detailWhen: "Wanneer u het nodig heeft",
      detailResult: "Wat u krijgt",
    },
    process: {
      eyebrow: "Hoe wij werken",
      title: "Een rustige, gestructureerde manier om ESG beheersbaar te maken.",
      description: "Vijf stappen, end-to-end of als losse opdracht. Iteratief van opzet, want ESG is geen eenmalig project.",
      cycleCta: "Iteratief van opzet",
      cycleTitle: "Gebouwd om te herhalen, niet om opnieuw te starten.",
      cycleDesc1: "ESG-verwachtingen, regelgeving en stakeholdervragen evolueren elk jaar. We zetten elke opdracht zo op dat het werk zich opbouwt - uw baseline, materialiteit, KPI's en disclosures worden scherper met elke cyclus.",
      cycleDesc2: "Dat betekent een lagere kost per cyclus, snellere antwoorden aan klanten en auditors, en een stille, duurzame ESG-capaciteit binnen uw bedrijf.",
      objective: "Doelstelling",
      deliverables: "Deliverables",
      standalone: "Beschikbaar als losse dienst",
      steps: [
        {
          title: "Uw context begrijpen",
          shortDesc: "Businessmodel, stakeholders, regelgevende blootstelling en wat écht materieel is.",
          objective: "Een scherp beeld opbouwen van uw bedrijf en de ESG-druk die er werkelijk toe doet.",
          deliverables: ["Context- & stakeholdermap", "Overzicht regelgevende blootstelling", "Werkhypothese over prioriteiten"],
        },
        {
          title: "De huidige baseline beoordelen",
          shortDesc: "Maturiteitscheck, data-review, leveranciers- en peer-benchmarking.",
          objective: "Eerlijk vaststellen waar u vandaag staat - zonder op te kloppen of te onderschatten.",
          deliverables: ["Maturiteitsbeoordeling", "Baseline data-review", "Peer- & leveranciersbenchmark"],
        },
        {
          title: "Prioriteren wat het meeste telt",
          shortDesc: "Materialiteit, ambitiebepaling en een gefaseerde roadmap.",
          objective: "Beslissen waarop te handelen, in welke volgorde en met welke diepgang - op basis van impact en haalbaarheid.",
          deliverables: ["Materialiteitsbeeld", "ESG-strategie & ambities", "Gefaseerde roadmap"],
        },
        {
          title: "Praktische volgende stappen uitvoeren",
          shortDesc: "Actieplannen, KPI's, governance en hands-on programmasupport.",
          objective: "Strategie omzetten in routines die uw team draait zonder dat wij elke week aan tafel moeten zitten.",
          deliverables: ["Actieplannen met eigenaars", "KPI-design & governance", "Interne capaciteitsopbouw"],
        },
        {
          title: "Rapportering & opvolging ondersteunen",
          shortDesc: "Rapportagestructuur, disclosure, klant- en frameworkantwoorden.",
          objective: "Het werk omzetten in heldere, verdedigbare communicatie - intern, naar klanten en in formele disclosures.",
          deliverables: ["Rapporteringsblauwdruk", "CSRD / VSME-afstemming", "Klant- & frameworkantwoorden"],
        },
      ],
    },
    why: {
      eyebrow: "Waarom Sustavision",
      title: "Boutique kwaliteit, op maat van hoe KMO's werkelijk werken.",
      reasons: [
        { title: "Op maat gemaakt voor uw bedrijf", description: "Elke opdracht wordt afgestemd op uw sector, omvang en maturiteit - geen kant-en-klare templates of hergebruikte draaiboeken." },
        { title: "Praktisch boven theoretisch", description: "Kaders waar ze helpen, gewoon werk waar ze dat niet doen. We optimaliseren voor wat uw team effectief kan uitvoeren." },
        { title: "Gemaakt voor lean teams", description: "Wij passen ons aan bedrijven zonder ESG-afdeling aan - minimale interne overhead, maximale voortgang." },
        { title: "Meerdere instappunten", description: "Strategie, rapportering, carbon, keten of implementatie. Start waar de druk zit, breid uit waar het loont." },
        { title: "EU- & marktkennis", description: "CSRD, VSME, EcoVadis, CDP, klantvragenlijsten - actuele verwachtingen, actuele taal, actuele onderbouwing." },
        { title: "Rustige, no-nonsense uitvoering", description: "Directe communicatie, eerlijke scoping en een werkstijl die uw tijd en uw bedrijf respecteert." },
      ],
    },
    experience: {
      title: "Ervaring in operationele sectoren",
      description: "We leverden ESG-ondersteuning in distributie-, productie- en logistiekgerelateerde contexten - onder andere. De sectoren hieronder zijn voorbeelden, geen beperkingen.",
      sectors: [
        { title: "Chemie & distributie" },
        { title: "Food & drinks-distributie" },
        { title: "Haven- & industriële operaties" },
        { title: "Productie" },
      ],
    },
    finalCta: {
      title: "Niet zeker waar te beginnen?",
      description: "In een kennismaking van 30 minuten bepalen we de echte prioriteit en of Sustavision de juiste fit is. Geen verplichtingen, geen generiek verkoopverhaal - gewoon een duidelijk beeld van wat zinvol is voor uw bedrijf.",
      cta: "Plan een kennismaking van 30 minuten",
      secondary: "Of mail Thomas op thomas@sustavision.com",
    },
    footer: {
      tagline: "Boutique ESG-advies voor KMO's.",
      positioning: "Heldere ESG-richting. Praktische uitvoering.",
      quickLinks: "Navigatie",
      contact: "Contact",
      email: "thomas@sustavision.com",
      phone: "+32 495 632 266",
      rights: "Alle rechten voorbehouden.",
    },
    about: {
      eyebrow: "Over ons",
      title: "Over Sustavision",
      bio1: "Sustavision wordt geleid door Thomas Huyghe. Ik help KMO's en groeiende bedrijven met ESG-vragen, klantverzoeken en rapportagedruk, zonder het uit te laten groeien tot een groot intern project.",
      bio2: "Ik behaalde een MSc in Supply Chain Management (Universiteit Antwerpen) en een MSc in Global Business & Sustainability (Rotterdam School of Management). Eerder bouwde ik bij BDO de sustainability-praktijk mee op en liet die groeien van 2 naar 13 consultants in 2,5 jaar. Bij Manuchar leidde ik carbon footprinting, responsible sourcing en de decarbonisatieroadmap, werk dat bijdroeg tot het behalen van EcoVadis Platinum.",
      bio3: "Sustavision bestaat om dat niveau van ESG-advies beschikbaar te maken voor KMO's en groeiende bedrijven, zonder de lagen, overhead of theoretische omweg van een groot kantoor.",
      bio4: "U krijgt senior, hands-on ondersteuning die is afgestemd op kleine teams en gericht op wat echt het verschil maakt voor uw bedrijf.",
      cta: "Plan een kennismaking van 30 minuten",
    },
    servicesPage: {
      title: "Diensten",
      description: "ESG wordt steeds meer een onderdeel van zakendoen: klantvragenlijsten, aanbestedingen en nieuwe regels. Wij helpen u om dit te doorgronden, geloofwaardig te antwoorden en praktische acties uit te voeren, zodat u zich op uw bedrijf kunt blijven richten.",
      includedLabel: "Wat is inbegrepen",
      outcomesLabel: "Typische resultaten",
      forLabel: "Voor wie is dit",
      discussCta: "Bespreek deze dienst",
      services: [
        {
          title: "Krijg helder wat ertoe doet",
          introduction: "U wilt begrijpen wat ESG werkelijk betekent voor uw bedrijf. Wij brengen vereisten, klantverwachtingen en interne prioriteiten in kaart en vertalen die vervolgens naar een gerichte roadmap die u ook echt kunt uitvoeren.",
          included: [
            "ESG-maturiteitsscan",
            "Analyse van verwachtingen van klanten, platformen en regelgevers",
            "CSRD-relevantiecheck en VSME-voorbereiding",
            "Materialiteitsanalyse (volledig of light-touch, naargelang uw noden)",
            "Duidelijke, geprioriteerde ESG-roadmap voor 12-18 maanden",
          ],
          outcomes: [
            "Helderheid over wat nu belangrijk is en wat later kan",
            "Een praktisch ESG-plan afgestemd op uw middelen",
            "Vertrouwen in gesprekken met klanten, banken of investeerders",
          ],
          forWhom: "Vooral nuttig voor bedrijven die steeds meer ESG-vragen krijgen, maar geen eigen duurzaamheidsfunctie hebben.",
        },
        {
          title: "Antwoord met vertrouwen",
          introduction: "Klanten, platformen en aanbestedingen vragen steeds vaker om ESG-informatie. Wij helpen u om heldere, geloofwaardige antwoorden voor te bereiden zonder onnodige interne werklast te creëren.",
          included: [
            "Ondersteuning bij EcoVadis, CDP en andere ESG-vragenlijsten",
            "ESG-datavragen van klanten en leveranciers",
            "Duurzaamheidsluiken in aanbestedingen en RFP's",
            "Verzamelen van bewijsstukken en documentatieondersteuning",
          ],
          outcomes: [
            "Sterkere, consistentere ESG-antwoorden",
            "Geordende bewijsstukken en documentatie",
            "Minder last-minute stress rond deadlines",
          ],
          forWhom: "Bedrijven die ESG-vragenlijsten of datavragen krijgen van klanten, platformen of aanbestedingen en praktische, deskundige ondersteuning nodig hebben.",
        },
        {
          title: "Zet plannen om in vooruitgang",
          introduction: "U wilt van ESG-intenties naar tastbare acties gaan. Wij helpen u om uw voetafdruk te meten, prioriteiten te bepalen en praktische verbeteringen door te voeren die bij uw bedrijf passen.",
          included: [
            "CO2-voetafdruk (Scope 1-3) en hotspotanalyse",
            "Praktische reductieprioriteiten en acties",
            "Leveranciersbetrokkenheid en dataverzameling",
            "ESG-KPI's en eenvoudige opvolgroutines",
            "Ondersteuning bij SBTi-afgestemde doelstellingen (waar relevant)",
          ],
          outcomes: [
            "Een eerste geloofwaardige CO2-baseline",
            "Duidelijke, uitvoerbare reductiestappen",
            "Betere ESG-data en betrokkenheid van leveranciers",
            "Eenvoudige routines die uw team kan volhouden",
          ],
          forWhom: "Bedrijven die hun milieu- en sociale impact willen meten en verbeteren, zonder het onnodig complex te maken.",
        },
      ],
      process: {
        title: "Hoe wij werken",
        description: "Gestructureerd, maar gericht op uw realiteit.",
        steps: [
          { title: "Uw situatie begrijpen", description: "We starten met uw context: klanten, sector, middelen en de ESG-vragen waarmee u te maken krijgt." },
          { title: "De juiste prioriteiten bepalen", description: "Samen bepalen we wat nu relevant is, wat kan wachten en hoe een realistische volgende stap eruitziet." },
          { title: "Praktische volgende stappen zetten", description: "We helpen u het werk uit te voeren, van vragenlijsten en roadmaps tot carbondata en leveranciersbetrokkenheid." },
        ],
      },
      finalCta: {
        title: "Niet zeker welke dienst past?",
        description: "In een kennismaking van 30 minuten bekijken we uw situatie en de ESG-vragen waarmee u te maken krijgt. U krijgt een helder beeld van wat een zinvolle volgende stap is, zonder verplichtingen en zonder generiek verkoopverhaal.",
        cta: "Plan een kennismaking van 30 minuten",
        secondary: "Of mail Thomas op thomas@sustavision.com",
      },
    },
    contact: {
      title: "Laten we praten.",
      description: "Vertel ons waar u staat en wat op uw bord ligt. We reageren persoonlijk, doorgaans binnen één werkdag.",
      formTitle: "Stuur een bericht",
      nameLabel: "Naam",
      namePlaceholder: "Uw naam",
      emailLabel: "E-mail",
      emailPlaceholder: "uw@bedrijf.com",
      messageLabel: "Bericht",
      messagePlaceholder: "Vertel kort over uw context en wat u wil oplossen.",
      submit: "Verstuur bericht",
      sending: "Verzenden…",
      success: "Bericht verzonden.",
      successDesc: "Bedankt. We nemen persoonlijk contact op, doorgaans binnen één werkdag.",
      error: "Er ging iets mis",
      errorDesc: "Probeer het opnieuw, of mail rechtstreeks naar thomas@sustavision.com.",
      directContact: "Of neem rechtstreeks contact op",
    },
  },
} as const;

export type Language = keyof typeof translations;
export type TranslationKey = keyof typeof translations.en;
